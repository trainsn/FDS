...
{	
	if(T1==NULL&&T2==NULL)/*If they're both NULL*/
		return 1;
	else if(T1==NULL||T2==NULL)/*If only one of them are NULL*/
		return 0;
	else if(T1->Element!=T2->Element)/*If they are both not NULL*/
		return 0;
	else return isequal(T1->Left,T2->Left)*isequal(T1->Right,T2->Right);
}
